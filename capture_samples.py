...
PALABRAS = [
    "me llamo", "yo", "tú", "él", "ella",
    "hola", "buen dia", "chau", "como estas?",
    "gracias", "querer", "comer", "casa",
    "ir", "preguntar", "amigo", "amiga", "ayudar"
]

if __name__ == "__main__":
    samples_per_word = 20  # Reducido a 20 muestras por palabra
    for word_name in PALABRAS:
        word_path = os.path.join(ROOT_PATH, FRAME_ACTIONS_PATH, word_name)
        os.makedirs(word_path, exist_ok=True)
        
        print(f"\n{'='*50}")
        print(f"GRABANDO MUESTRAS PARA: {word_name.upper()}")
        print(f"{'='*50}")
        
        for sample_num in range(1, samples_per_word + 1):
            print(f"\nMuestra {sample_num} de {samples_per_word} - Palabra: {word_name}")
