...
def interpolate_frames(frames, target_frame_count=15):
    ...
    # Si no hay frames, retornar frames vacíos
    if current_frame_count == 0:
        return [np.zeros((224, 224, 3), dtype=np.uint8)] * target_frame_count
    
    # Si solo hay 1 frame, replicarlo
    if current_frame_count == 1:
        return [frames[0]] * target_frame_count
        
    # Si ya tiene la cantidad deseada, retornar tal cual
    ...
    for i in indices:
        ...
        lower_idx = min(lower_idx, current_frame_count - 1)
        upper_idx = min(upper_idx, current_frame_count - 1)
        
        if lower_idx == upper_idx:
            interpolated_frame = frames[lower_idx]
        else:
            weight = i - lower_idx
            interpolated_frame = cv2.addWeighted(
                frames[lower_idx], 1 - weight, 
                frames[upper_idx], weight, 0
            )
