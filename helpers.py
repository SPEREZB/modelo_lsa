...
def get_sequences_and_labels(words_id):
    ...
    for word_index, word_id in enumerate(words_id):
        ...
        try:
            ...
            # Si los datos tienen la columna 'keypoints' directamente
            if 'keypoints' in data.columns:
                # Tomar hasta MODEL_FRAMES frames
                seq_keypoints = data['keypoints'].values[:MODEL_FRAMES]
                # Rellenar con ceros si es necesario
                if len(seq_keypoints) < MODEL_FRAMES:
                    seq_keypoints = np.pad(seq_keypoints, 
                                        (0, MODEL_FRAMES - len(seq_keypoints)), 
                                        'constant', 
                                        constant_values=0)
                sequences.append(seq_keypoints)
                labels.append(word_index)
            # Si los datos tienen la estructura antigua con 'sample'
            ...
            print(f"Advertencia: No se encontró el archivo {hdf_path}")
            continue
