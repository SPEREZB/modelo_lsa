...
def training_model(model_path, epochs=500):
    ...
    if sequences:
        print(f"Tipo del primer elemento: {type(sequences[0])}")
        print(f"Longitud del primer elemento: {len(sequences[0]) if hasattr(sequences[0], '__len__') else 'No tiene longitud'}")
        if hasattr(sequences[0], 'shape'):
            print(f"Forma del primer elemento: {sequences[0].shape}")
